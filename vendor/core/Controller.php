...
require './vendor/libs/bistro/Hash.php';
require './vendor/libs/bistro/Session.php';
require './vendor/libs/bistro/Native.php';
...
abstract class Controller
{
	...
	public static function output($content, $type = 'text/html', $codeHttp = 200, $allowMinify = true) {
		...
		header('X-Powered-By: ' . \blitz\vendor\Bootstrap::$settings['app']['name'] . \blitz\vendor\Bootstrap::$settings['app']['author']);
		header('Expires: max-age=0');
		header('Cache-Control: no-cache, no-store, must-revalidate');
		header('Pragma: no-cache');
		header('Content-Language: ' . \blitz\vendor\Bootstrap::$settings['locale']);
		header('Server: ' . \blitz\vendor\Bootstrap::$settings['app']['name']);

		if ($type !== null) {
			header("Content-type: {$type}; charset=UTF-8");
		}

		header($_SERVER['SERVER_PROTOCOL'] . ' ' . $codeHttp . ' ' . \blitz\vendor\Bootstrap::$settings['http_code_list'][$codeHttp] . ' ' . \blitz\vendor\Bootstrap::$settings['app']['name'], true, $codeHttp);
